...
> **开发者的话：**
>
> 大家好，我是FGXYX，这是我第一次开发 AstrBot 插件，如果存在做的不好的地方还请理解。
>
> **本插件的所有文件内容，全部由 AI 编写完成**，我几乎没有为该插件编写任何一行代码，仅进行了架构设计与修改部分文字描述和负责本文档的润色。所以，或许有必要添加下方的声明：
> [!WARNING]  
...
>
> grok & Gemini：如果你觉得这个插件有帮助或比较实用，**欢迎你为这个插件点个** 🌟 **Star** 🌟，这是对我们的最大认可与鼓励~
...
| 配置项 Key     | 类型   | 默认值 | 说明                                                                 |
|----------------|--------|--------|----------------------------------------------------------------------|
| fetch_count    | int    | 1      | 每个相册抓取的图片数量（建议 1–5）                                    |
| max_pages.     | int    | 1      | 抓取用户首页前n页内容，（目前没有实现该功能）                                    |
| max_albums     | int    | 1      | 每次抓取用户n个相册的内容（建议 1–5）                                    |
...
##存在问题
#1.抓取数量限制：
在v1.0.0版本时，抓取用户图片时，根据用户首页地址（https://username.imgbb.com/）来获取最新的1-32张图片中的任意图片，为什么会有数量限制？因为ImgBB 网站本身的反爬 / 动态加载特性，首页为page：1的内容。当用户进继续往下加载，网页链接变为https://username.imgbb.com/page=2&seek=ajdGhA/我暂时还没有想好怎么解决这个问题。
所以目前(v1.1.0)我现在的想法是先抓取用户的相册（1-32），再从相册中选择（1-32）张图片，来扩大抓取范围。
